...
OUT_FILE     = os.path.join(DIR, "Raw_Material_Usage_Output_NEW.xlsx")

IN_SCOPE = {"Hot Chick", "WTF", "Korea Town", "Wing Fest", "Kuro Smash"}
...
def write_xsheet(ws, df, col_widths, row_color_fn=None, default_fmt=None):
    """Write a DataFrame to an xlsxwriter worksheet. row_color_fn(row_dict) → hex or None."""
    ...
    for ri, row in enumerate(df.itertuples(index=False), start=1):
        ...
        for ci, val in enumerate(row):
            ...
            ws.write(ri, ci, v, rfmt)

# ── Sheet 1: Dashboard ────────────────────────────────────────────────────────
ws_dash = xwb.add_worksheet("Dashboard")
ws_dash.set_column(0, 0, 36); ws_dash